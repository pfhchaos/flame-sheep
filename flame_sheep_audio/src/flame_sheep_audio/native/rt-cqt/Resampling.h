	...
	*/
	...
	inline std::vector<double> HalfBandLowpass<FloatType, AllpassNumber>::filterDesign()
	{
		...
		for (size_t i = 1; i <= ((n - 1) / 2); i++)
		{
			...
			while (delta > 1.e-100)
			{
				delta = std::pow((-1.), m) * std::pow(q, (m * (m + 1.))) * std::sin((2. * m + 1.) * audio_utils::Pi<double>() * static_cast<double>(i) / static_cast<double>(n));
				num += delta;
				m += 1.;
			}
			...
			while (delta > 1.e-100)
			{
				delta = std::pow((-1.), m) * std::pow(q, std::pow(m, 2.)) * std::cos(2. * m * audio_utils::Pi<double>() * static_cast<double>(i) / static_cast<double>(n));
				den += delta;
				m += 1.;
			}
			den = den * 2. + 1.;
