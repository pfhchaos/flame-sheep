...
class TestTimelineBuffer:
    ...
    def test_push_and_retrieve(self, timeline):
        now = 1000.0
        timeline.push(_make_event('low', 0.8, now))
        marks = timeline.marks_in_window(now)
        assert len(marks['low']) == 1
        assert marks['low'][0].energy == 0.8
    ...
    def test_window_filtering(self, timeline):
        ...
        timeline.push(_make_event('low', 0.5, 990.0))  # 10s ago
        timeline.push(_make_event('low', 0.8, 997.0))  # 3s ago
        timeline.push(_make_event('low', 1.0, 999.5))  # 0.5s ago
        marks = timeline.marks_in_window(1000.0)
        # Window is 4s, so 990.0 should be excluded
        assert len(marks['low']) == 2

    def test_clear(self, timeline):
        timeline.push(_make_event('low', 1.0, 1000.0))
        timeline.clear()
        marks = timeline.marks_in_window(1000.0)
        assert len(marks['low']) == 0

    def test_multiple_bands(self, timeline):
        now = 1000.0
        timeline.push(_make_event('low', 1.0, now))
        timeline.push(_make_event('mid', 0.7, now))
        timeline.push(_make_event('high', 0.5, now))
        marks = timeline.marks_in_window(now)
        assert len(marks['low']) == 1
        assert len(marks['mid']) == 1
        assert len(marks['high']) == 1
...
def _snap(**kwargs) -> AudioSnapshot:
    ...
    defaults = dict(
        ...
        break_intensity=0.0,
        section_change=0.0,
        bands={'low': BandState(rms=0.1, onset_density=2.0, density_delta=0.1),
               'mid': BandState(rms=0.05, onset_density=1.0),
               'high': BandState(rms=0.02, onset_density=4.0),
...
class TestHeaderPanel:
    ...
    def test_update_reads_state(self):
        ...
        panel.update(snap, TimelineBuffer(['low']), 0.016)
