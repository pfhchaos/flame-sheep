...
def _ensure_schema(conn: sqlite3.Connection):
    ...
    for col in ('symmetry_max', 'rotational', 'reflective', 'radial', 'periodic', 'fractal_dim'):
...
class Library:
    ...
    def genome_scores(self, genome_id: int) -> dict[str, float]:
        ...
        row = self.conn.execute(
            'SELECT coverage, entropy, color_entropy, balance, complexity FROM genomes WHERE id = ?',
            (genome_id,),
        ).fetchone()
        if row is None:
            raise KeyError(f'No genome with id {genome_id}')
